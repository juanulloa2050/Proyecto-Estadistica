...
```{r}
imprimir_tabla_frecuencia <- function(data, columna, titulo = NULL) {
  # Calcular frecuencia absoluta y relativa
  frec_abs <- table(data[[columna]])
  frec_rel <- prop.table(frec_abs)

  # Crear dataframe con los resultados
  tabla_frecuencia <- data.frame(
    Valor = names(frec_abs),
    Frecuencia_Absoluta = as.vector(frec_abs),
    Frecuencia_Relativa = as.vector(frec_rel)
  )

  # Crear el objeto de tabla con gt()
  tabla_gt <- gt(tabla_frecuencia) %>%
    tab_header(
      title = titulo
    ) %>%
    tab_spanner(label = "Frecuencia", columns = c(Frecuencia_Absoluta, Frecuencia_Relativa))
...
```{r}
imprimir_tabla_frecuencia(Base_de_Datos_Encuesta_IA, "carrera", "carrera")
